...
_Code review du 2026-08-10, sur un diff de 1007 lignes couvrant la phase `publish` **et** les 12 patches de la review 23.5, jamais relus. Le Blind Hunter a rendu ; **l'Acceptance Auditor n'a pas été lancé** (calé 3 fois sur 3 dans la session, son travail est fait à la main) et **l'Edge Case Hunter a calé** (11 min de silence, tué) — sa question la plus utile (« qu'est-ce qui change pour les utilisateurs ») a été reprise au navigateur. Audit d'acceptation : **les 9 AC sont satisfaits**, vérifiés contre le code et la base._

**Cinq constats graves, dont quatre portent sur du code que j'ai écrit hier soir en corrigeant 23.5.**

- [x] [Review][Patch] **Le garde d'écriture peut faire perdre TOUS les champs d'une fiche, définitivement** — `computeFill` traite `''`, `'  '`, `[]` et `{}` comme des trous, mais `buildFillGuard` n'émet que `IS NULL` (ou `IS NULL OR = ''` pour trois champs texte). Une fiche dont `mode = ''` ou `genre = '[]'` produit donc un `where` qui ne matche **rien** : les 7 autres champs prêts à être écrits sont perdus avec, la fiche part dans `raced` avec le message « publiée ou remplie depuis la sélection » — qui est faux — et **chaque relance refait exactement la même chose**. ⚠️ **Atteignable sur tes données** : la base contient **31 fiches avec une chaîne vide** et **15 avec un tableau vide**. Correctif : construire le garde depuis la valeur observée au SELECT (`{[Op.or]: [null, valeurObservée]}`), pas depuis une hypothèse sur ce qu'est le vide.
- [x] [Review][Patch] **`mode` manque dans `ENRICH_TEXT_FIELDS`** — j'ai listé `album`, `key`, `timeSignature` et oublié `mode`, qui est pourtant la colonne texte la plus susceptible de valoir `''`. Elle n'est donc ni trimée ni gardée correctement : c'est le déclencheur direct du constat précédent.
- [x] [Review][Patch] **Une publication à moitié faite n'est réparable par AUCUNE phase** — `publish` ne sélectionne que les brouillons et `enrich` aussi. Si la re-synchronisation échoue après la publication, la fiche n'est plus un brouillon : plus personne ne la retrouvera, et ses détenteurs gardent la bannière **pour toujours**. Le rapport affiche « ⚠️ ces gens verront la bannière » sans offrir le moindre remède. J'ai pourtant vanté l'auto-réparation d'`enrich` — `publish` ne l'a pas. Correctif : que la requête de candidats retourne **aussi** les fiches publiées dont une chanson a dérivé.
- [x] [Review][Patch] **Une exception pendant la re-synchronisation compte la fiche en « publiée » ET en « échec », et l'omet des « désynchronisées »** — `report.published += 1` s'exécute avant `Song.update` ; si celui-ci lève, le `catch` pousse dans `failed`. La fiche apparaît donc dans deux totaux contradictoires et **manque dans la seule liste qui sert à savoir qui verra la bannière**. `enrich` fait cette distinction et la teste ; `publish` ne la fait pas, et son test d'erreur ne rejette que l'écriture de la fiche, jamais la re-synchronisation.
- [x] [Review][Patch] **`has_data` compte `''` et `[]` comme de la donnée** — la requête fait `colonne IS NOT NULL`, ce qui contredit `isEmptyValue` utilisé partout ailleurs, et le commentaire qui rappelle trois cents lignes plus haut que « `key = ''` n'est pas NULL ». Une fiche dont le seul champ non-nul est `key = ''` est donc **exclue** de la liste « À REMPLIR » — alors que c'est exactement la coquille que cette liste doit nommer.
- [x] [Review][Patch] **`has_data` s'appuie sur `ENRICH_FIELDS`, dont le sens est différent, et le test qui protégeait ce couplage a été supprimé** — `ENRICH_FIELDS` veut dire « ce qui peut voyager d'une chanson vers une fiche », pas « ce qui rend une fiche non vide ». Mon propre commentaire invite à y réintroduire `pitchStandard` un jour : ce jour-là, `pitch_standard IS NOT NULL` est vrai pour les 125 fiches, `has_data` devient constamment vrai et la liste « À REMPLIR » se vide en silence. C'est précisément ce que `ENRICH_SIGNAL_FIELDS` empêchait avant que je le supprime. Correctif : une constante propre à `publish`, plus un test qu'une colonne à valeur par défaut ne peut jamais rendre `has_data` vrai.
- [x] [Review][Patch] **Le dry-run de `publish` ne peut pas annoncer le seul chiffre qui compte** — `resynced` vaut toujours 0 en dry-run, donc la ligne « marqueurs resynchro. » ne s'affiche jamais et l'opérateur n'a aucune réponse à « combien de personnes vais-je re-estampiller ». Le dry-run d'`enrich`, lui, le compte : les deux phases se contredisent.
- [x] [Review][Patch] **Le dry-run d'`enrich` sous-compte les re-synchronisations** — la branche « rien à combler » incrémente `resynced`, mais celle des fiches qui **seront** écrites sort sans le faire, alors qu'une écriture garantit un `updatedAt` neuf donc une re-synchronisation. Le dry-run annonce 2, l'apply en fait 75.
- [x] [Review][Patch] **Le chemin d'auto-réparation estampille un horodatage lu au SELECT** — potentiellement vieux de plusieurs minutes sur un lot de 125. Si un curateur édite la fiche entre-temps, on pose l'ancienne valeur et on rapporte un succès, alors que la fiche est déjà repartie devant. Le chemin d'écriture utilise `returning` précisément pour éviter ça ; le chemin de réparation, non.
- [x] [Review][Patch] **La raison d'un rejet est perdue quand TOUTES les valeurs sont refusées** — le `push` dans `report.dropped` est situé après la sortie anticipée « rien à combler ». L'opérateur lit « source REFUSÉE : 1 (bpm) » sans savoir si c'était hors bornes ou de forme invalide, alors qu'une fiche à un bon champ **plus** un refusé, elle, affiche la raison complète.
- [x] [Review][Patch] **« DÉSYNCHRONISÉES : fiche écrite, marqueur PAS reposé » est faux sur le chemin de réparation** — là, rien n'a été écrit dans la fiche ; seule la réparation a échoué. Le message envoie chercher une modification qui n'existe pas.
- [x] [Review][Patch] **Mon test « LE CŒUR » ne teste pas ce qu'il annonce** — il affirme `report.resynced === 3`, mais 3 est simplement ce que le mock a été programmé pour renvoyer ; le `songCount: 3` du candidat est décoratif et l'assertion passerait même si le code ignorait complètement ce compte. Et `selectPublishCandidates` — qui porte la déduplication, `Number(song_count)` et `!!has_data` — n'est testé nulle part.
- [x] [Review][Defer] **Rien n'impose que `publish` tourne après `enrich`** [`backend/scripts/seed-catalog.js`] — deferred. Lancer `--phase=publish --apply` en premier publierait 75 fiches vides que `enrich` ne retrouverait plus jamais. L'ordre est documenté, non contraint. Report assumé : le script est manuel, la séquence est écrite dans l'en-tête et dans le rapport de 23.4, et la contrainte demanderait un état persistant que ce script n'a pas.
- [x] [Review][Defer] **Pas de `--limit`, pas de `--only`, pas de confirmation avant de rendre 75 fiches publiques d'un coup** [`backend/scripts/seed-catalog.js`] — deferred. Dépublier se fait à la main. Report assumé pour cette exécution-ci : le dry-run donne le compte exact avant, et northwood veut précisément le passage unique. À reconsidérer si le script resert sur un volume plus gros.
- [x] [Review][Defer] **Troncature à la milliseconde entre le `Date` JS et le microseconde Postgres** [`backend/scripts/seed-catalog.js`] — deferred. Inoffensif tant que la drift se calcule en JS (les deux côtés tronquent pareil), mais une comparaison faite en SQL rendrait `fiche.updatedAt > marqueur` vrai pour toutes les chansons touchées. À garder en tête si une requête de liste calcule la drift côté base un jour.

**Patches appliqués le 2026-08-10** — les 12 constats `patch` sont corrigés. Backend **527 → 543 tests**, lint propre.

**Les deux corrections structurelles, prouvées sur données réelles :**

- *Le garde d'écriture juge désormais sur la valeur **observée** au SELECT* (`{[Op.or]: [null, valeurVue]}`) au lieu de supposer que « vide » veut dire `NULL`. Une fiche à `mode = ''` ne fait donc plus perdre ses sept autres champs à chaque relance.
- *`publish` est devenu réparable* : sa requête ramène aussi les fiches **déjà publiques** dont une chanson a dérivé. Vérifié en cassant volontairement deux marqueurs sur des fiches publiées — l'ancienne version n'aurait **rien** pu faire (zéro brouillon en base) ; la nouvelle les retrouve, les répare (`⟲ marqueur réparé`), et la drift repasse de 2 à 0.

**Trois mutations ont survécu au premier passage** — donc trois patches n'étaient couverts par aucun test. Deux sont désormais tués (le comptage du dry-run enrich, la relecture d'horodatage à la réparation). Le troisième ne peut pas l'être : ajouter `mode` à `ENRICH_TEXT_FIELDS` est **inerte**, parce que `normalizeMode` trime déjà. C'est écrit tel quel dans le code plutôt que déguisé en test qui passerait de toute façon — le vrai correctif de ce constat était le garde.

**Un défaut trouvé par l'exécution finale** : le rapport annonçait « fiches brouillon : 2 » pour deux fiches **publiées** en réparation. Corrigé en « fiches à traiter ».

_Écarté comme bruit (1) : « `Op` n'est peut-être pas importé, ce qui ferait 75 faux échecs » — vérifié, il l'est bien (`const { fn, col, where: whereFn, Op, QueryTypes } = require('sequelize')`)._
