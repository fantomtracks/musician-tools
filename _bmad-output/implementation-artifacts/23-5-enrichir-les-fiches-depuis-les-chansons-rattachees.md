...
- **Contrôle navigateur non refait après l'enrichissement** : la session s'est fermée entre-temps et je ne saisis pas de mot de passe. Ce qui le remplace : la bannière est conditionnée à la drift (`CatalogSourceBanner.tsx:109`), mesurée à **0** après l'exécution, et l'enrichissement ne touche pas `published_at` — les fiches restent brouillon, donc `getSong` n'émet toujours aucune provenance. À reconfirmer d'un coup d'œil avant la prod.
