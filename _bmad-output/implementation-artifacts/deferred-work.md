...
> **Rituel (acté rétro Epic 8, 2026-06-21)** : ce fichier passe en revue **avant le démarrage de chaque nouvelle epic**. Chaque item est tranché — _fix maintenant_ / _story planifiée_ / _gardé-avec-raison_ / _tué_. Il ne doit jamais redevenir un cimetière. Dernière passe : **2026-07-05 (post-rétro Epic 15)** — **A5 marqué résolu** (livré Epic 15 : 15.1 signal UX 429 + 15.2 oracle de timing, NFR-S1+S4) ; **6 entrées 7.x déjà résolues** par les lots ménage/session/parité **barrées** pour lisibilité ; tous les autres résidus de review + le bloc § 🧊 **re-confirmés gardés** (mono-user beta) ; **rien promu** — seul candidat « vraie landing page » **non retenu** cette fois (pause décidée) ; rien à fixer en urgence. Passe précédente : 2026-07-05 (post-rétro Epic 14 — A5 promu, devenu Epic 15). Avant : 2026-07-04 (lot UX/UI mobile promu → Epic 14).
...
- **7-13 — Oracle de timing sur `resendVerificationPublic`** — 🔗 **RATTACHÉ à l'epic A5 (2026-07-05, cf. § À brainstormer)** : l'envoi email n'est awaité (avant `res.json`) que sur la branche existant-non-vérifié → la latence distingue « compte non vérifié » de « inconnu/vérifié » malgré le body uniforme. Même résidu accepté que `forgotPassword`, à traiter avec le cadrage anti-oracle A5. [`backend/controllers/usercontroller.js:260-279`]
