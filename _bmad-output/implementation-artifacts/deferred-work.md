...
- **[DATA-LOSS-MED/HIGH — fenêtre étroite] Refresh-from-Catalog peut écraser des edits non sauvegardés** — sur la fiche Song, le `CatalogSourceBanner` fait le refresh **sans flusher l'autosave en attente**. Fenêtre : drift affiché + l'utilisateur tape (ex. `notes`) + clique **Refresh** avant que le débounce autosave (~1,2 s) parte. Le refresh serveur tourne alors sur la ligne DB **ancienne** (les notes tapées pas encore sauvées), renvoie ces vieilles valeurs perso, et `onRefreshed → buildFormFromSong(updated)` **réécrit le form** (les notes tapées sont perdues, la baseline autosave repasse à l'ancien). Un autosave déjà en vol au moment du refresh peut aussi **rétablir l'ancienne baseline** après le reseed. **Réel mais fenêtre étroite** (débounce court + faut drift+edit+refresh simultanés). **Vrai fix** (à faire délibérément, zone autosave délicate de `Songs.tsx`) : soit **flusher l'autosave en attente AVANT le refresh** (prop `beforeRefresh` → `flushAutoSave`), soit **ne reseed que les champs intrinsèques** du form (préserver les champs perso non sauvés). Relevé par la review 21.2 (Edge, Med/High). [src/components/CatalogSourceBanner.tsx `handleRefresh`, src/pages/Songs.tsx autosave/`buildFormFromSong`]
