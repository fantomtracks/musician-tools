...
> **Rituel (acté rétro Epic 8, 2026-06-21)** : ce fichier passe en revue **avant le démarrage de chaque nouvelle epic**. Chaque item est tranché — _fix maintenant_ / _story planifiée_ / _gardé-avec-raison_ / _tué_. Il ne doit jamais redevenir un cimetière. Dernière passe : **2026-07-10 (pré-nouvelle epic, post-Epic 16)** — **3 items soldés/barrés** : bug artiste non-trimmé (livré Epic 16 / 16.1), tuning basse demi-ton (livré Epic 16 / 16.2), résidu 7-13 oracle de timing `resendVerificationPublic` (résolu Epic 15 / 15.2) ; **lot UX/UI mobile barré** (livré Epic 14) ; nouveau report review 16.1 (titre legacy tout-espaces) **tranché → gardé-avec-raison** (pré-existant, hors périmètre, mono-user) ; tous les autres résidus de review + le bloc § 🧊 **re-confirmés gardés** (mono-user beta) ; **rien promu** — candidats « vraie landing page » + « auto-création fiche chanson » restent à cadrer (brainstorm/UX avant code) ; rien à fixer en urgence. Passe précédente : 2026-07-05 (post-rétro Epic 15 — A5 marqué résolu, livré Epic 15). Avant : 2026-07-05 (post-rétro Epic 14 — A5 promu, devenu Epic 15) ; 2026-07-04 (lot UX/UI mobile promu → Epic 14).
...
- **17-2 — Bouton back navigateur / popstate non gardé par le guard de nav** : `<BrowserRouter>` n'expose pas de blocker react-router → seuls les clics in-app (`GuardedLink`) et le `beforeunload` sont couverts ; un back-button hors d'un draft titre-vide « à valeur » n'affiche pas la popup et ne supprime pas un draft fresh. 🧊 **Gardé-avec-raison** (limite déjà documentée dans la story ; migrer vers un data-router serait une refonte routing dédiée). [`src/main.tsx` BrowserRouter]
