...
- **[UX] Dup-check Catalog proactif best-effort (peut rater sur un titre courant)** — le blocage de doublon au form Catalog fait `listCatalog({ search: title, includeDrafts: true, limit: 10 })` + `find` sur la clé foldée. `search` étant une recherche **substring paginée**, un doublon exact `(titre, artiste)` sur un titre courant (ex. « Love ») peut tomber **hors des 10 premières lignes** → non détecté proactivement → l'autosave part et le **backstop 409** (index unique GLOBAL serveur) bloque quand même (~1.2s + 1 requête ratée). **Correctness garantie** (aucun doublon créé) ; seul le « bloquer/masquer Publish dès la frappe » est best-effort. Amélioration = un endpoint léger de check **exact** `(title, artist)` (réutiliser `findExistingByTitleArtist` côté serveur) → blocage fiable et immédiat. [src/pages/CatalogAdmin.tsx, backend/controllers/catalogcontroller.js]
