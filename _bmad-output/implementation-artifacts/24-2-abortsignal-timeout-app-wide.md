...
- ✅ **le câblage d'une surface est désormais verrouillé** : `CatalogManage.test.tsx` rend la page dans le `GlobalToastProvider`, abandonne un lot de suppression en cours et assère le récap. Mutation : retirer le `showGlobalToast` **tue 1 test**. Les deux autres surfaces (add-to-collection, remove-from-collection) suivent le même motif et restent à couvrir ;
