...
|------|---------|-------------|--------|
| 2026-08-11 | 0.3 | Suite de la review : une URL de base absente ne produit plus une pile Sequelize (`The "url" argument must be of type string`) mais un message qui nomme l'environnement ET la variable attendue. Constat du Blind Hunter d'abord reporté, puis **observé en vrai** en démarrant l'image de production sans secrets — d'où sa promotion. La table env → variable vit dans `db.js` pour le seul message, `config.js` restant la source de vérité ; un **test anti-dérive** empêche les deux de diverger, sinon on recréait la duplication que cette story venait de supprimer. Backend 568 → 572, 2 gardes vérifiés par mutation (témoin neutre vert), message confirmé dans le conteneur réel. | northwood |
