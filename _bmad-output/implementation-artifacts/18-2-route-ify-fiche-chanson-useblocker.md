...
- Fix #2 : garde auto-création lit `pathnameRef`/`isMountedRef` (refs live) au lieu du `location` figé dans la closure → plus de yank-back en quittant `/songs/new` par lien header / back navigateur pendant un create en vol.
  ...
  - **Suivi QA manuelle #3 (2026-07-11)** : vider le titre d'une chanson **fraîche** (juste créée, rien d'autre) et quitter ouvrait le popup au lieu de supprimer en silence. Cause : `isFreshSong` comparait le `form` à `initialSong` par `JSON.stringify` exact, mais le `form` est reconstruit depuis l'**objet serveur** (porte `uid`/`userUid`/timestamps/`instrumentLinks`/`streamingLinks` et des `null` là où `initialSong` a `''`) → la comparaison échouait **toujours** pour une chanson chargée du serveur (le « fresh silent delete » ne marchait jamais en vrai ; le test 17.2 passait avec un mock minimal). Corrigé : `isFreshSong` teste désormais l'**absence de contenu** champ par champ (helper `isEmptyValue`, `null`/`''`/`[]`/`{}` = vide, `pitchStandard` 440 = défaut), en ignorant les métadonnées serveur. Test de non-régression déterministe (record serveur avec métadonnées + `null`), vérifié qu'il échoue sur HEAD. Fix bonus : isolation du mock `getAllSongs` dans `beforeEach`. Front 362✓.
